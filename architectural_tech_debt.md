...
| 2 | A3 | Pagination helper for list reads — *helper shipped + read-path adoption Story 10.16* | Code | 2 | 3 | 3 | **15** |
...
- **Known limitation (unchanged, not introduced):** `depcheck._PYPROJECT` and `tools/_log.py`'s `LOG_FILE` are still derived from `__file__` and so only resolve correctly in a source tree / editable install — under a wheel install they point outside the package, exactly as they did under the flat layout. `depcheck`'s module docstring already scopes it to "the source tree of an editable install"; the depth changed, the contract did not.
