...
from shopify_mcp.shopify.operations import inventory as ops
from shopify_mcp.shopify.queries.inventory import (
    GET_INVENTORY_ITEM,
    GET_PRODUCT_INVENTORY,
    SET_INVENTORY,
    UPDATE_INVENTORY_ITEM_TRACKED,
)
from shopify_mcp.tools._filters import filter_variant_targets
...
def register(server: FastMCP, client: ShopifyClient) -> None:
    ...
    def update_variant_inventory_tracking(
        ...
        header = _outcome_header("Variant inventory tracking update", len(changed), len(failed))
        return (
            f"{header}\n"
            f"  Product : {title} (id: {product_id})\n"
            f"  Target  : tracked={tracked}\n"
