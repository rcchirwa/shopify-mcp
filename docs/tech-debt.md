...
- The three **anchor** positions (`<`, `/`, `>`) admit only their own confusables, derived from Unicode *names* inside the punctuation and symbol categories — a name carrying `SOLIDUS`/`SLASH` but not `REVERSE`, one carrying `LESS-THAN` or a left-pointing `ANGLE`, one carrying `GREATER-THAN` or a right-pointing one. ASCII is excluded from all three by construction.
- Every **letter** position and the interior **separator** admit the ASCII character or *any ink-rendering non-ASCII character at all* — `_INK`, the complement of the invisible and whitespace sets.

The anchors buy specificity and the interior buys completeness, which is what keys the rule on the delimiter's **shape** rather than on the presence of a confusable — the card's design guard. Real shopper copy is full of Cyrillic letters, CJK angle brackets and fraction slashes; what it does not contain is a bracket-shaped character, then a solidus-shaped one, then nine ink characters, then a separator, then four more, then a closing bracket. Because the interior is a complement rather than an enumeration, there is **no homoglyph table to curate and no script left uncovered** — Greek, Armenian, Cherokee and the mathematical alphanumerics are in scope for free, and nothing goes stale when Unicode adds a lookalike. That answers the card's derived-tripwire requirement by avoiding hand-curation rather than by guarding it.

**Two facts that had to be derived rather than assumed, and one that corrects the card.** First, a name-derived *dash* class does **not** subsume `_DASH_CONFUSABLES`: U+2015 HORIZONTAL BAR carries neither `HYPHEN`, `DASH` nor `MINUS` in its name. SEC-21's hand-curated list is therefore kept in the separator position — now redundant against `_INK`, but retained deliberately as the executable record of SEC-21's empirical finding and as the thing that keeps that position correct if the ink allowance is ever narrowed. Second, **not every forgery is answered by neutralization, and the tests had to stop asserting that it is.** U+2F03 KANGXI RADICAL SLASH matches the raw scan but NFKC-folds to the CJK ideograph U+4E3F, so `wrap()` emits the normalized copy, which no longer spells the delimiter at all; U+3372 SQUARE APAATO does the same at a letter position, expanding to four kana. Both are exactly as safe as a backslash, and the sweeps assert the security property — *the forgery does not come back byte-for-byte* — rather than the mechanism.
...
**Non-vacuity was proven, not asserted.** The three tests that encode the new behaviour were re-run against pre-fix `_untrusted.py` and watched fail — the Evidence test on the missing backslash, the name-derived anchor sweep, and the letter sweep reporting 425,923 surviving codepoints. `count("</UNTRUSTED-DATA>") == 1` is deliberately not load-bearing anywhere in the new tests: Story 10.70's review caught that shape as vacuous, and it reads 1 before and after the fix alike.

**Regex cost held flat, and import cost with it.** The pattern is still one compiled regex whose every quantified run is a single character class separated by mandatory single-character classes; the anchor and ink classes are disjoint from the gap class by construction, so the linearity argument is unchanged in form — a start position is now bounded by the next `<`-*like* character rather than the next literal `<`. Deriving five classes instead of one would have tripled import cost (60 ms → 180 ms) had the whitespace regex kept running over all 0x110000 codepoints; it is now asked only inside `Cc`/`Zs`/`Zl`/`Zp`, which a sweep confirms are the only categories `\s` ever matches and a new test pins. Measured 85 ms.

**Known residual, recorded rather than fixed.** A span whose interior is *entirely* non-ASCII still matches, though it reads as foreign text in brackets rather than as the delimiter — it needs a bracket-shaped character immediately followed by a solidus-shaped one, then fourteen ink characters, then a closing bracket, which is why no realistic copy tested hits it. Requiring at least one interior position to be its ASCII character would exclude it, at the cost of a counting rule a single-pass regex cannot express and a threshold with no principled value. **Entry condition:** a false positive observed on real store content, or any report of `wrap()` altering a value that contains no delimiter. Damage if it fires is bounded — one inserted backslash plus the NFKC fold Story 10.63 narrowed away — and would be visible in a description round-trip.

Full gate chain green: `shopify-mcp-check-deps`, `ruff check`, `ruff format --check`, `mypy`, **1812 offline tests at 100% coverage** (`--fail-under=100`), and `pip-audit --strict` clean on both `requirements.lock` and `requirements-dev.lock`.

### Closed

| # | Item | How it closed |
|---|------|----------------|
| SEC-21-confusables | ~~Visible-glyph confusables of `<`, `/`, `>` that NFKC does not fold (U+2215, U+2044, U+29F8, U+2039/U+203A, U+3008/U+3009, U+276E/U+276F) and homoglyph letters inside `UNTRUSTED`/`DATA` (Cyrillic А/Т) spelled a visually-identical closing delimiter that `_CLOSE_TAG_PATTERN` did not match — the visible half of SEC-21's coverage, which reasoned about NFKC per-position and never enumerated the bracket/slash confusables NFKC leaves alone~~ | Closed by splitting the pattern's positions into **narrow name-derived anchors and a broad complement-derived interior**. `<`, `/` and `>` admit only their own confusables, derived from Unicode names within the punctuation/symbol categories with ASCII and reversed forms excluded; every letter position and the separator admit the ASCII character or any ink-rendering non-ASCII character, `_INK` being the complement of the invisible and whitespace sets. That split is what keys the rule on the whole delimiter shape rather than on the presence of a confusable, and it removes hand-curation entirely rather than guarding it — no homoglyph table, no script uncovered, nothing to go stale as Unicode grows. Three behavioral sweeps pin it: every non-ASCII codepoint that NFKC-folds to `<`/`/`/`>` (a derivation deliberately independent of the module's), every name-derived angle/solidus codepoint at its position, and every ink-category non-ASCII codepoint at three letter positions — the last reporting 425,923 survivors against pre-fix code. Negative tests hold the false-positive line on Cyrillic, CJK-in-`〈〉`, Greek-in-`‹›`, a U+2044 fabric spec, a bracketed URL and raw `body_html`. The interior-entirely-non-ASCII residual is documented above with its entry condition. `wrap()`'s control flow untouched; all Story 10.63 and 10.70 guards intact, 10.63's five-spelling test unmodified. Full gate chain green, 1812 offline tests at 100% coverage, pip-audit clean on both lockfiles. |
