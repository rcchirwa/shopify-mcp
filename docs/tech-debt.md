...
3. **The both-supplied refusal stops at this module's boundary — `T-10.65-refuse-both-fanout`.** Seven other tools take the same two `product_id` / `handle` parameters and still apply a silent `product_id`-wins precedence: `products.py`'s `get_product`, `get_product_description`, `get_product_full` (via `ops/products.py`, whose docstring states "``product_id`` wins when both are given") and the four `publications.py` tools (via `ops/publications.py`). The hazard used to justify break 2 above — a caller who names one product by ID and a *different* one by handle gets data for the first with no indication the second was ignored — **is still live on those seven**. Not fixed here: they live in two other modules with their own contracts and test suites, and widening a refusal across nine tools is a change that deserves its own card rather than being appended to this one. Recorded so the claim in this entry is read as module-scoped, which is how the code and the `_identifier_channel` docstring now state it. Flagged by the round-2 deep review, which correctly caught that an earlier revision of the README row and docstrings asserted the rule held "on every tool that resolves a product".
