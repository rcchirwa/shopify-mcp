...
**Which guarantee this buys, stated plainly (Implementation step 3).** Approach 1 makes the wrong-product write **avoidable everywhere** — every mutator now has a channel on which an all-digit handle resolves correctly. It does **not** make it *impossible*: a caller who still puts an all-digit handle into `product_id` gets bare-digit → numeric-ID classification and writes to whichever product carries that ID. That is the same limitation 10.64 accepted, now with an escape hatch on all six tools instead of one. Only approach 2 (or approach 3's endpoint) closes it, and that remains gated on usage evidence nobody has yet.
...
1. **The both-supplied trap.** `product_id` wins when both identifiers are given, so a client that fills *both* slots with the same all-digit handle (a plausible LLM-client behaviour: one identifier, two parameters) still resolves it as a numeric ID and still writes to the wrong product. `_resolve_product` itself would raise on both-supplied; `_identifier_channel` deliberately swallows that conflict to implement the precedence. This precedence is what the card's approach 1 specifies and what `get_product_metafields` has documented since 10.64, so it was **not** changed here on review feedback — but it is the one wrong-product-write path this story leaves reachable *through the new parameter*, and rejecting both-supplied on the five write tools would close it at no backward-compatibility cost, since `handle` is new on all five. Revisit alongside the approach-2 question.
