...
- **The unfenced-title surface.** Warnings were placed to avoid widening it; fencing titles remains a convention-wide change, never a change made here alone.

### Residuals, recorded not fixed

- **`inventoryLevels(first: 10)` in `shopify/queries/inventory.py` has the identical silent shape**, nested inside a paginated `variants` connection with no `pageInfo`, at two sites. A variant stocked at more than 10 locations drops levels from `get_inventory` with no signal. **Found while writing this story's card and recorded in no ledger before 2026-09-05.** Not fixed here because it is a different domain and a different tool, and practical exposure today is nil: CLAUDE.md records that no `read_locations` scope is granted and that location work is manual admin. Entry condition: a store with more than 10 stock locations, or `read_locations` ever being granted.
