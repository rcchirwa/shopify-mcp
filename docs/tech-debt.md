...
**Two adjacent correctness gaps fixed in passing, because the new channel exposed them.** Making `product_id` optional meant every message that echoed it verbatim would render blank on a handle-only call: `update_product_vendor` / `update_product_type` / `update_product_options`' "no product found for ''", `update_product_options`' "is not on product ''", and `update_variant_image_binding`'s "No product found with id ." plus its five preview/confirm/log head lines. All now report whichever identifier the caller actually supplied. Separately, giving `product_id` a default forces defaults onto the parameters after it, which would have turned an omitted `product_type` into a **silent field clear** (`""` is the documented clear value). `product_type` is therefore defaulted to `None`, not `""` — `_normalize_product_type` already rejects `None` with "product_type is required (pass '' to clear the field)", so omission is now a clean error instead of a destructive no-argument write. `option` is defaulted to `None` for the same reason and is already rejected by `_normalize_option_input`.
