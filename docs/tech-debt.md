...
**Accepted residual — recorded, not silently left open, and narrower than it first reads.** Approach 3 makes the ambiguity expressible and avoidable **only where a tool actually exposes the `handle` channel**. Today that is `get_product_metafields` alone — the one tool that already had the parameter. The five mutators that resolve a product (`update_product_category`, `update_product_vendor`, `update_product_type`, `update_product_options`, `update_variant_image_binding`) still take `product_id` only, so for them there is *no* alternative channel and the wrong-product **write** — the severe outcome this item was raised for — remains reachable exactly as before. Bare digits in `product_id` still mean "numeric ID": that is the deliberate cost of backward compatibility and of preserving the zero-network-call path, since a handle-first lookup would add a round-trip to *every* numeric call. Threading `handle` through the mutator signatures is a public-surface change to five tools and is left to a follow-up story, not smuggled into this one. The trade-off is documented at the `isdigit()` branch, in `_resolve_product`'s docstring, and in `get_product_metafields`' parameter docs.
