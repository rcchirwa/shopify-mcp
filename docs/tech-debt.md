...
2. **The neither-supplied message does not mention `handle`.** All five surface `"product_id must be a non-empty string."`, while the sibling `get_product_metafields` says `"At least one of product_id or handle is required."` — inconsistent guidance for the same caller mistake. Not fixed here: an omitted `product_id` and an explicit `product_id=""` are indistinguishable at this layer, so improving the wording necessarily changes the error for `product_id=""` too, and that exact string is pinned by pre-existing tests and documented as mapped **verbatim** by `_resolve_product_gid` for its other callers (see T-9.5-resolver-fanout difference #3). Changing it is a contract change to those callers, not a wording tweak.
3. **`update_variant_image_binding`'s "No product found with id `<ref>`" now says "id" when the ref is a handle.** Left as-is: the wording pre-dates this story, is pinned by a pre-existing test, and is already tracked as a watched behavioural string under T-9.6-error-msg-shift — a third shift of the same message to fix an imprecision is not worth the churn.

**The one deliberate breaking change — `get_product_metafields` no longer applies `product_id`-wins.** Every other change in this story is additive: `handle` is new on the five mutators, so no caller can be passing both there, and the keyword-only conversion only affects positional Python calls that MCP never makes. This one is different. `get_product_metafields` has carried *both* parameters since before 10.64, and 10.64 explicitly documented and pinned the precedence — so a caller supplying both today gets a structured error where it previously got data. Recorded as a genuine break rather than filed under the additive changes.
