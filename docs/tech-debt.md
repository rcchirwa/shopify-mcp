...
**The shared-helper question, answered explicitly because the card asked for it on the record: promoted, not paralleled.** `_require_discriminator` was private to `operations/products.py`, so `operations/publications.py` could not reuse it. The rule now lives once in a new `shopify/_identifiers.py` (`reject_both_identifiers` + `BOTH_IDENTIFIERS_ERROR`), imported by both operations modules **and** by `tools/_product_resolver.py`, so catalog_hygiene's refusal and these seven tools' refusal are literally the same sentence rather than two that merely resemble each other. It sits under `shopify/` rather than `tools/` because `shopify/` must never import `tools/` — pinned by `tests/architecture/test_layering.py`.
