...
**One genuine hole was found that capping alone would not have closed.** `update_product_pricing` called `ops.read_variants_for_pricing` with **no** `try/except` — the only product read in the module unguarded. A transport or Shopify error propagated out of the tool and FastMCP rendered the raw exception, so the unbounded upstream body still reached model context after every reflection site was capped. Capping the tool's two local-validation sites while leaving that open would have been a half-fix that passed its own tests. Now wrapped, matching every sibling tool. **Deliberate contract change:** the tool returns a structured error where it previously raised. No test pinned the raising behaviour, and every other tool in the module already behaved this way.
...
**The invariant is now executable, not a one-time sweep — with a stated blind spot.** `test_s1067_no_uncapped_exception_reflection_remains_in_the_tool_surface` walks the four reflecting modules and fails on any `{e}`/`{exc}`/`str(e)` interpolation not routed through `cap`/`_cap` (skipping `raise` sites, which are bounded at their reflection point instead). The module-by-module split arose precisely because nothing stopped a new uncapped site from being added; a per-tool behavioural test cannot catch the next one, and this does. **It is a line-scoped regex, so it is a tripwire, not a proof:** it would miss `{e!s}`, `{exc.args}`, `"%s" % e`, or an interpolation split across lines from its `cap()`. None of those forms exist in the four modules today (checked), and the per-tool behavioural tests cover the paths that matter end-to-end — but do not read a green invariant as "capping is provably total". 23 new tests total — per-tool head and JSON-tail assertions in the paired positive+negative style (`cap(huge) in out` **and** `huge[:MAX+1] not in out`), so a refactor that drops the reflection entirely cannot pass vacuously.

### Closed

| # | Item | How it closed |
|---|------|----------------|
| SEC-27 | ~~`docs/tech-debt.md` held two live, contradictory positions on whether reflected exception text is capped — SEC-resolver-reflect-cap declared it "intentionally left uncapped", SEC-11 recorded capping it in the sibling modules — and the code split the same way, `catalog_hygiene.py` (0 of 37 sites capped) against `publications.py`/`inventory.py`/`media/_upload.py` (all capped). The repo could not answer its own question, so every security pass re-found and re-litigated it~~ | Capping adopted repo-wide at `REFLECT_MAX_LEN` (300). Every caller-facing exception interpolation in `catalog_hygiene.py` routes through `cap(str(...))` — 35 lines rewritten in the first pass, with the two `raise ValueError(f"… {exc}")` sites deliberately excluded (they are bounded at the reflection point that catches them) and two producer-bounded sites carrying an inline `# reflect-ok:` marker. Deliberately stated as a rule with named exceptions rather than an absolute count: the grep total moves whenever a guard is added, and an off-by-N in this entry is exactly the kind of wrong doc claim that caused SEC-27 in the first place; the other three modules were already compliant and are untouched. The contradicting sentence is struck in place with the retraction reasoned, not silently deleted. Bound chosen per value-class (identifiers 200, exception text 300) and recorded. `update_product_pricing`'s unguarded `ops.read_variants_for_pricing` — the one path where an unbounded body escaped uncaught — is now wrapped, a deliberate raise→structured-error contract change. Approach 3 (cap in `client.py`) declined on the evidence that no unbounded consumer remains, recorded rather than left implicit. 23 tests, each watched fail first: per-tool head and JSON-tail assertions in the paired positive+negative style; an occurrence-granular invariant over the **whole** tool surface plus `client.py` (the first cut scanned a hardcoded four-file allowlist, which is how `collections.py` escaped it) with inline `# reflect-ok: <reason>` exemptions that a companion test requires to carry a justification; and an AST walk asserting no `ops.*` call is left unguarded. |
