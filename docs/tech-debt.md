...
Closes the last open piece of SEC-04. Story 10.41 wrapped metafield values and media alt text and deliberately left three surfaces alone, asking a later story to *decide* rather than mechanically port. Trello: https://trello.com/c/taD9JsTD (Story 10.63, Epic 10). **The decision was made and written here before any code was edited** (card Implementation step 4); what follows is the reasoning as it stood at decision time, not a post-hoc justification.

**Decision: wrap. All four description read surfaces, plus the stored-value half of all three write-preview echoes.**
...
**Step 3's question — can descriptions be written by a path the sanitizer does not cover — answered yes, and it is not close.** `sanitize_html` is applied at exactly three call sites, all inside this server (`update_product_description`, `update_product_seo`, `update_collection`). The Shopify Admin UI, any other installed app, the Admin API used directly, a CSV/bulk import, and a supplier or dropship feed all write `descriptionHtml` without passing through it. A write-path control in one client cannot protect a read path fed by every other client of the same store. This is why the read-side wrapper is the correct layer even in the counterfactual where `nh3` *did* catch prose.
...
**The usability cost the deferral feared was overestimated, and that is worth recording because it was the other half of the "against" case.** `wrap()` adds one opening and one closing delimiter — ~35 characters — plus a one-line reminder header, and preserves the payload verbatim (neutralized, never dropped or truncated). A multi-KB description is not made materially less legible by 35 characters at its ends. There was no real trade to make here.
...
- **`seo.title` / `seo.description`.** Rendered raw by `get_product_full`. Same class of merchant-authored free text, and a defensible next target — but not named in 10.41's deferral, so not claimed here. Note it is a *narrower* surface than `bodyHtml` (short, and `update_product_seo` already routes through `sanitize_html`), which is why it does not change this story's priority ordering.
- **The `` ```json `` tail.** Unchanged by construction, per 10.41's split. None of the seven wrapped sites lives in a tool that emits a JSON tail — `catalog_hygiene.py` is the only module that renders one (`_render`), and it has no description or `bodyHtml` surface at all (verified by grep, not assumed). The tail contract is therefore preserved trivially, and the AC's "proven by test" is honoured by pinning the *full* output of each affected tool rather than by asserting a fence is absent — a test that only checked for the absence of a ` ```json ` fence would pass for the wrong reason and prove nothing.

### Closed

| # | Item | How it closed |
|---|------|----------------|
| SEC-04-descriptions | ~~The residual Story 10.41 (SEC-04) deferred: product/collection descriptions returned raw to the model, and the `update_product_media` write-preview echo unwrapped — the widest remaining unlabelled free-text surface in the server~~ | Decided (not mechanically ported) and recorded above before any edit: **wrap**, on the finding that the deferral's central counter-argument was false — `nh3` leaves a prose injection payload byte-identical and unflagged, verified empirically, so the write-path sanitizer does not reduce this risk at all. Applied the existing `tools/_untrusted.py::wrap()` at seven sites across `tools/products.py`, `tools/collections.py`, and `tools/media/_update.py`; no hand-rolled delimiters introduced, so SEC-18/SEC-21's closing-tag neutralization covers every new site for free. Conditional behaviour mirrors SEC-04 exactly: an empty or absent value renders with no wrapper, and `INJECTION_REMINDER` is prefixed only when the emitted output actually contains a wrapped value. The three write previews' **old/new asymmetry** is the substantive correction — 10.41 dismissed the media echo as "mostly caller-supplied input", but every preview renders an old/new pair whose *old* half is stored store content, and for `update_product_media` it is the same `alt` field `list_product_media` has wrapped since SEC-04; the stored halves are now wrapped and the caller-supplied halves deliberately left raw. Error-text fencing, which SEC-27's entry had assigned to this card, is explicitly declined and re-carded with the reasoning above. Full gate chain green, coverage 100%. |
...
- ~~**Product / collection descriptions and the `update_product_media` write-preview echo** — deferred; not in the recommended priority. The write-preview echo mostly reflects caller-supplied input, and descriptions are a larger free-text surface worth their own scoped story if the injection risk is later judged material.~~ **Closed 2026-08-17 by Story 10.63 (SEC-04-descriptions) — see the entry at the top of this file.** The "worth their own scoped story" call was right and the risk was judged material. Two claims in the struck text were wrong and are corrected there rather than here: the echo does **not** mostly reflect caller-supplied input (each write preview renders an old/new pair whose *old* half is stored store content), and the later-added reliance on `nh3` making stored descriptions "less hostile" does not hold — `nh3` leaves prose injection payloads byte-identical.
