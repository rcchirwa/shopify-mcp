...
**Accepted residual — recorded, not silently left open.** Approach 3 makes the ambiguity *expressible and avoidable*; it does not make the wrong-product write *impossible*. Bare digits in `product_id` still mean "numeric ID", so a caller who passes a numeric handle through `product_id` still resolves to the wrong product silently. That is the deliberate cost of backward compatibility and of preserving the zero-network-call path for numeric/GID input — a handle-first lookup would add a round-trip to *every* numeric call. The trade-off is documented at the `isdigit()` branch, in `_resolve_product`'s docstring, and in `get_product_metafields`' parameter docs.

### Closed

| # | Item | How it closed |
|---|------|----------------|
| T-9.5-numeric-handle | ~~`_resolve_product` classified `product_id` as numeric-ID-vs-handle purely via `stripped.isdigit()`, so an all-digit Shopify handle was misread as a legacy product ID and silently resolved to the wrong product (or none); `str.isdigit()` additionally accepted non-ASCII digits and produced malformed GIDs~~ | `_resolve_product` gains a keyword-only `handle` parameter — the unambiguous channel that runs `productByHandle` with **no** numeric classification; `product_id` becomes optional and keeps its exact historical semantics (bare digits → numeric ID, GID passthrough, otherwise handle), so every existing caller is untouched. Supplying both raises `ValueError` before any network call rather than applying a precedence rule. The duplicated handle-lookup body is extracted to `_lookup_by_handle`. The numeric test is tightened to `stripped.isascii() and stripped.isdigit()`; non-ASCII digits now fall through to the handle lookup and simply do not resolve — deliberately **not** a new exception type, so `_resolve_product_gid`'s adapter contract is untouched. `catalog_hygiene._resolve_product_gid` gains a matching keyword-only `handle` passthrough and reports whichever identifier the caller supplied in its not-found message; its historical `(gid, error_str)` shape, its five error strings, and all four of its callers are otherwise unchanged. **Confirmed live bug fixed:** `get_product_metafields` documented `handle` as "Resolved via productByHandle before the metafield query" but collapsed both parameters into `_resolve_product_gid(client, pid or hdl)` — an all-digit handle supplied through that tool's own documented, unambiguous parameter was classified as a numeric ID and read the wrong product, i.e. a caller doing everything right got silently bad data. It now routes `handle` through the keyword channel, preserving the documented `product_id`-wins precedence. 13 new tests (`test_product_resolver.py`: handle-channel dispatch with and without a snapshot query, stripping, not-found, both-supplied rejection, backward-compat numeric short-circuit, historical empty-input message, parametrized non-ASCII digit cases, ASCII regression; `test_catalog_hygiene.py`: the all-digit-handle regression, the numeric-`product_id` zero-lookup guard, and `product_id`-wins precedence) — each watched fail against the pre-fix tree before implementation. |
