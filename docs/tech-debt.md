...
**The rule, stated once so nothing has to re-derive it: exception text reflected to a caller or a log is capped at `REFLECT_MAX_LEN` (300), everywhere.** `tools/_scrub.cap` is the only slicing implementation; `catalog_hygiene._cap` remains a thin 200-char wrapper for *identifiers*, which is a different class of value and is unchanged. Trello: https://trello.com/c/jFyHjFrA (Story 10.67, Epic 10).
...
**Approach 3 (cap inside `client.py::_format_errors`) was NOT folded in, and the reason is evidential rather than a scope dodge.** Every consumer that reflects a `ShopifyError` to a caller or a log now caps at the reflection point: `catalog_hygiene` (this story), `publications`, `inventory`, `media/_upload`, and `tools/_log.py` via its own `MAX_DESC_LEN`. With no unbounded consumer left, capping again at construction would be redundant while changing a component every tool depends on. Recorded so the next security pass sees a decision, not an oversight. Note `client.py` *could* do it — it already imports `tools/_scrub` — so this is a choice, not a layering constraint.
...
**The invariant is now executable, not a one-time sweep.** `test_s1067_no_uncapped_exception_reflection_remains_in_the_tool_surface` walks the four reflecting modules and fails on any `{e}`/`{exc}`/`str(e)` interpolation not routed through `cap`/`_cap` (skipping `raise` sites, which are bounded at their reflection point instead). The module-by-module split arose precisely because nothing stopped a new uncapped site from being added; a per-tool behavioural test cannot catch the next one, and this does. 16 new tests total — per-tool head and JSON-tail assertions in the paired positive+negative style (`cap(huge) in out` **and** `huge[:MAX+1] not in out`), so a refactor that drops the reflection entirely cannot pass vacuously.

### Closed

| # | Item | How it closed |
|---|------|----------------|
| SEC-27 | ~~`docs/tech-debt.md` held two live, contradictory positions on whether reflected exception text is capped — SEC-resolver-reflect-cap declared it "intentionally left uncapped", SEC-11 recorded capping it in the sibling modules — and the code split the same way, `catalog_hygiene.py` (0 of 37 sites capped) against `publications.py`/`inventory.py`/`media/_upload.py` (all capped). The repo could not answer its own question, so every security pass re-found and re-litigated it~~ | Capping adopted repo-wide at `REFLECT_MAX_LEN` (300). All 37 interpolation sites in `catalog_hygiene.py` route through `cap(str(...))`; the other three modules were already compliant and are untouched. The contradicting sentence is struck in place with the retraction reasoned, not silently deleted. Bound chosen per value-class (identifiers 200, exception text 300) and recorded. `update_product_pricing`'s unguarded `ops.read_variants_for_pricing` — the one path where an unbounded body escaped uncaught — is now wrapped, a deliberate raise→structured-error contract change. Approach 3 (cap in `client.py`) declined on the evidence that no unbounded consumer remains, recorded rather than left implicit. 16 tests, each watched fail first, including an executable invariant over all four modules so the split cannot silently reappear. |
